...
winstreak_obs_winprob <- lapply(seq(1,10), function(i){
  ...
  mutate(gchan = ifelse(!game_changed, "No change", ifelse(risk_increased, "Risk increased", "Risk decreased"))) %>%
  mutate(game_never_changed = streak >= game_streak) %>%
  group_by(winstreak, label, gchan) %>%
...
ggplot(winstreak_obs_winprob, aes(x=streak, y=obs_pwin, group=interaction(winstreak, label))) +
  geom_line(aes(color = label, linetype = label), size = 1)+
  geom_point(aes(shape = winstreak), size = 2.2) + facet_wrap(vars(gchan))+
...
ggplot(pwin_change_prob,aes(fill=status, y=change_prob, x=streak)) +
  ...
  theme_light() +
  scale_fill_manual(values = c("#E7033C","#13C062")) +
  theme(
    axis.title.x = element_text(colour="black", size = 13),
    axis.title.y = element_text(colour="black", size = 13),
    axis.text.y = element_text(size = 12),
  ...
  scale_y_continuous(labels=scales::percent_format(accuracy = 5L), limits = c(.0,.5)) +
  labs(
    x = "Streak length",
    y = "Probability of change in game",
    fill = "Streak type:")
...
setwd(WORKDIR)
ggsave("plots/figure_4_3.0.pdf", device = "pdf", width = 18.26,  height = 12.63,  units = "cm")
```
# Size of pwin change
...
pwin_change_relative <- pooled_table_unscaled %>% 
  ...
  summarize(
    no_changer_ratio = sum(streak<game_streak)/n(), 
            .groups = "drop")

ggplot(data = pwin_change_relative, aes(x = factor(streak_signed), y = no_changer_ratio, group = label)) +
  geom_point(aes(color = label, shape = label), size = 4)

pwin_change_relative <- pooled_table_unscaled %>% 
  filter((streak < 7) & game_changed) %>%
  mutate(streak_signed = ifelse(status == "win",streak,-streak)) %>%
  group_by(streak_signed, label, risk_increased) %>%
  summarize(
    game_diff_mean = mean(game_diff),
    game_diff_median = median(game_diff), 
            .groups = "drop")

ggplot(data = pwin_change_relative, aes(x = factor(streak_signed), y = abs(game_diff_mean), group = label)) +
  geom_point(aes(color = label, shape = label), size = 4) +
  facet_grid(vars(ifelse(risk_increased, "More risk", "Less risk")))
...
pwin_change_relative_boot %>%
  ...
  geom_vline(xintercept = which(levels(factor(.$streak_signed)) %in% '0')) +
    ...
    labs(
      ...
      #y = "Mean of relative change in winning probability",
      y = latex2exp::TeX("Mean log relative change in winning probabilities"),
      color = "Sample:",
      shape = "Sample:")}
