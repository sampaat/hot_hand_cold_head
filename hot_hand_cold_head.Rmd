...
setwd(WORKDIR)
ggsave("plots/figure_2_2.0.pdf", device = "pdf", width = 36.52,  height = 12.63,  units = "cm")
```
...
setwd(WORKDIR)
ggsave("plots/figure_3_2.0.pdf", device = "pdf", width = 18.26,  height = 12.63,  units = "cm")
```
...
setwd(WORKDIR)
ggsave("plots/figure_4_2.0.pdf", device = "pdf", width = 18.26,  height = 12.63,  units = "cm")
```
# Size of pwin change
...
pwin_change_relative %>%
  {ggplot(data = ., aes(x = factor(streak_signed), y = game_diff_logmean, group = label)) +
  ...
  geom_vline(xintercept = which(levels(factor(.$streak_signed)) %in% '0')) +
    ...
    labs(
      ...
      y = latex2exp::TeX("Change in winning probabilities"),
      color = "Sample:",
      shape = "Sample:")}

setwd(WORKDIR)
ggsave("plots/figure_5_2.6.pdf", device = "pdf", width = 18.26,  height = 12.63,  units = "cm")
```
...
setwd(WORKDIR)
ggsave("plots/figure_1_2.0.pdf", device = "pdf", width = 36.52,  height = 12.63,  units = "cm")
```
