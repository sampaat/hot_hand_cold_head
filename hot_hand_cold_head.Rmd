...
if(list.files(file.path(WORKDIR, "calcdata")) %>% grepl("^satoshidice_bets_matched_",.) %>% sum() < 5 ){print("Some input files may be missing!... Check README.md")}
```
...
# Within-person changes in betting probabilities
...
model_player_control_lag_only_changers <- fixest::feols(
  game_diff_ratio_log ~ win5 + lose5 | userID,
  data = game_changer_set %>% filter(streak != game_streak),
  cluster = ~userID
)

summary(model_player_control_lag_only_changers)
```