...
required_packages <- c("tidyverse", "rmarkdown", "ggpubr", "fixest")
...
```
# Prechecks for file system setup

If the following block provides any error messages, please action on them.

```{r echo=FALSE, cache=FALSE, results=FALSE, warning=FALSE, comment=FALSE, warning=FALSE}
WORKDIR <- "C:/Users/sampa/Documents/bce-bitluck" #set this for your own working directory

if(!dir.exists(file.path(WORKDIR, "calcdata"))){print("Inpud data directory 'calcdata' missing!...")}

if(!dir.exists(file.path(WORKDIR, "plots"))){print("Plot dump directory 'plots' missing!...")}

is_writable_dir <- function(dir) {
  test_file <- file.path(dir, paste0(".write_test_", Sys.getpid()))
...
res <- is_writable_dir(file.path(WORKDIR, "plots"))

if (!res$ok) {
  stop("WORKDIR/plots not writable: ", res$error)
}

#last check to see if files exist
setwd(WORKDIR)

files <- paste0(
  "calcdata/satoshidice_bets_matched__",
  c("2012-05-02","2012-09-17","2012-12-17","2013-05-04","2013-09-11"),
  "_to_",
  c("2012-05-22","2012-10-07","2013-01-06","2013-05-24","2013-10-01"),
  "_clean.csv"
)
...
Important quantities created (for this article that is):
  ...
  * label - identifier of the sample, bascially the observed period, check the bitcoin_pricehistory_lowvar.R script or the article for the dates
...
message("✔ Precalculations complete.")
```

# Summary statistics and Observed distribution of risk choices
...
#bitcoin prices used to be be added from the bitcoin_pricehistory_lowvar.R script
#the bitcoin price data has been turned proprietary in the meantime by bitcoincharts.org from public
...
# Population-wide observed winning probabilities after winning streaks of different lengths

On streak of n means the game followed n-1 games without changing the outcome at any. Off streak of n means the game has followed n-1 games of any but the same outcomes as the current one. Here we calculate the ratio of wins to losses following game histories on/off streak of n. The plot produces figure 1.
...
ggsave("plots/figure_2_2.2.pdf", device = "pdf", width = 29,  height = 35,  units = "cm")
```

# Mean chosen winning probability after winning and losing streaks of length n.

Similar to what we have observed before, but here we are measuring the median of the pwin, which is the implied probability of win (reciprocate of odds/multiplier) after an n long win/loss streak. The plot produces figure 2.
...
# Population-wide probabilities of increasing, decreasing, or holding the chosen winning probability after winning and losing streaks.
...
ggsave("plots/figure_4_5.2.pdf", device = "pdf", width = 20,  height = 28,  units = "cm")
```
# Means of relative logarithmic changes in chosen winning probabilities by streak length and observation period.

Here we calculate the relative size of change in pwin. Here -1 means halving pwin compared to the current game (e.g. from 50% to 25%) and +1 means doubling it (e.g. from 1% to 2%). Logarithmic scale is used to normalize the distribution of the ratios, a well known technique from finance. This section produces figure 4.
...
ggsave("plots/figure_6_2.1.pdf", device = "pdf", width = 20,  height = 28,  units = "cm")
```

# Fixed-effects regressions of within-person changes in chosen winning probabilities

We set up the indicator variables signaling the user experiencing long winning or losing streaks and game_diff_ratio_log is the same thing we used above at _Size of pwin change_. For the regression we will use feols from fixest to be able to controll over users.

```{r}
game_changer_set <- pooled_table_unscaled %>% 
  select(userID, pwin, pwin_pred, game_diff_ratio, streak, status, label, game_streak) %>%
  mutate(win3 = ifelse((streak > 2)&(status == "win"), 1, 0)) %>%
  mutate(lose3 = ifelse((streak > 2)&(status == "loss"), 1, 0)) %>%
  mutate(win4 = ifelse((streak > 3)&(status == "win"), 1, 0)) %>%
  mutate(lose4 = ifelse((streak > 3)&(status == "loss"), 1, 0)) %>%
  mutate(win5 = ifelse((streak > 4)&(status == "win"), 1, 0)) %>%
  mutate(lose5 = ifelse((streak > 4)&(status == "loss"), 1, 0)) %>%
  mutate(win6 = ifelse((streak > 5)&(status == "win"), 1, 0)) %>%
  mutate(lose6 = ifelse((streak > 5)&(status == "loss"), 1, 0)) %>%
  mutate(win7 = ifelse((streak > 6)&(status == "win"), 1, 0)) %>%
  mutate(lose7 = ifelse((streak > 6)&(status == "loss"), 1, 0)) %>%
  mutate(game_diff_ratio_log = log(game_diff_ratio+1, base = 2))
```

Models over different cutoff lengths of "long streak"

```{r}
model_player_control_lag_only_changers_3 <- fixest::feols(
  game_diff_ratio_log ~ win3 + lose3 | userID,
  data = game_changer_set %>% filter(streak != game_streak),
  cluster = ~userID
)

summary(model_player_control_lag_only_changers_3)
```

```{r}
model_player_control_lag_only_changers_4 <- fixest::feols(
  game_diff_ratio_log ~ win4 + lose4 | userID,
  data = game_changer_set %>% filter(streak != game_streak),
  cluster = ~userID
)

summary(model_player_control_lag_only_changers_4)
```

The results below are the ones summarized in Table 3.

```{r}
model_player_control_lag_only_changers_5 <- fixest::feols(
  game_diff_ratio_log ~ win5 + lose5 | userID,
  data = game_changer_set %>% filter(streak != game_streak),
  cluster = ~userID
)

summary(model_player_control_lag_only_changers_5)
```

```{r}
model_player_control_lag_only_changers_6 <- fixest::feols(
  game_diff_ratio_log ~ win6 + lose6 | userID,
  data = game_changer_set %>% filter(streak != game_streak),
  cluster = ~userID
)

summary(model_player_control_lag_only_changers_6)
```

```{r}
model_player_control_lag_only_changers_7 <- fixest::feols(
  game_diff_ratio_log ~ win7 + lose7 | userID,
  data = game_changer_set %>% filter(streak != game_streak),
  cluster = ~userID
)

summary(model_player_control_lag_only_changers_7)
```
Summarizing coefficient values for different lenghts: Coefficient estimates in Table A1.

```{r}
c(model_player_control_lag_only_changers_3$coefficients,
  model_player_control_lag_only_changers_4$coefficients,
  model_player_control_lag_only_changers_5$coefficients,
  model_player_control_lag_only_changers_6$coefficients,
  model_player_control_lag_only_changers_7$coefficients) %>%
  matrix(nrow = 2, dimnames = list(c("win","loss"),c("3","4","5","6","7")))
```
Summarizing coefficient standard errors for different lenghts: Coefficient estimate standard errors in Table A1.
```{r}
c(model_player_control_lag_only_changers_3$se,
  model_player_control_lag_only_changers_4$se,
  model_player_control_lag_only_changers_5$se,
  model_player_control_lag_only_changers_6$se,
  model_player_control_lag_only_changers_7$se) %>%
  matrix(nrow = 2, dimnames = list(c("win","loss"),c("3","4","5","6","7")))
```

# Empirical winning frequencies of players with and without streaks of different lengths

Here we are basically generating random sequences of outcomes for players who choose a random game from their initial set and stuck to it for n_games. Then we calculate the same observed winning probabilites Xu and Harwey worked with and plot it like we did for the data in the beginning. It produces figure A1.
